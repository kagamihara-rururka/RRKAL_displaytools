...
$packet = [ordered]@{
    ...
    has_view_model_render_plan = $coreSource -like "*rrkal_displaytools.view_model_render_plan.v1*"
    has_sample_view_model_render_plans = $coreSource -like "*rrkal_displaytools.sample_view_model_render_plans.v1*"
    has_sample_renderer_adapters = ($coreSource -like "*class DisplayToolsGeoRendererAdapter*") -and ($coreSource -like "*class ContractOnlyTimeSeriesRendererAdapter*")
    ...
    exports_contracts = ($initSource -like "*build_display_shell_capability_packet*") -and ($initSource -like "*build_canvas_registry_packet*") -and ($initSource -like "*build_sample_view_models_packet*") -and ($initSource -like "*build_view_model_render_plan_packet*") -and ($initSource -like "*build_sample_view_model_render_plans_packet*")
    sample_view_ids = @("rrkal_displaytools_earth_view_sample", "rrkal_displaytools_time_series_view_sample")
    sample_renderer_adapters = @("DisplayToolsGeoRendererAdapter", "ContractOnlyTimeSeriesRendererAdapter")
    sample_render_plan_schemas = @("rrkal_displaytools.view_model_render_plan.v1", "rrkal_displaytools.sample_view_model_render_plans.v1")
