...
- `rrkal_displaytools.layer_render_plan_compose_pass_budget.v1` records the current sequential overlay pass model, the target collapsed-run / single Taichi composite pass model, required zero-diff parity evidence and the Qt `composePassBudgetStrip` readiness summary. This keeps the user-visible performance plan aligned with the actual runtime blocker before merge optimization is enabled.
