...
- `rrkal_displaytools.ocean_material_control_port.v1` exposes the Qt Properties wave/roughness/foam controls, renderer flags, Taichi scalar uniforms and manual scalar sea-state port boundary. It now carries `rrkal_displaytools.ocean_material_renderer_apply_contract.v1`, `rrkal_displaytools.sea_state_scalar_sample.v1` and `rrkal_displaytools.ocean_material_summary_contract.v1`, pinning renderer apply and copyable handoff summaries to profile/launch/timeline scalar fields while keeping provider discovery, download, import and cache governance outside this repo.
