...
- Layer capability matrix 下一步可把 badge 狀態納入可複製的研究 provenance，並補更細 renderer diagnostics。
...
- Smoke 目前檢查：
  ...
  - Launch packet `layer_runtime_evidence` / `layer_runtime_evidence_summary` schema gate。
  ...
  - Handoff inspection `layer_capability_matrix` / `layer_runtime_evidence_summary` contract gate。
