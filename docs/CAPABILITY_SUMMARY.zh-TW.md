...
- Timeline 下一步是從 camera/ocean interpolation、PNG frame sequence export 與 GIF fallback 推進到 MP4/container video encoding 與 non-material interpolation；目前 Timeline dock、UI-only keyframe storage/restore/playback controls、profile save/load、No-GUI keyframe handoff、`timeline_state` status contract、active step handoff、renderer step playback、ocean/material interpolation、camera keyframe interpolation、discrete camera keyframe apply、PNG frame export 與 GIF animation export 已建立。
