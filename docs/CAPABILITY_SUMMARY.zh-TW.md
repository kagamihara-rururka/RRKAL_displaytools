...
- `rrkal_displaytools.layer_render_plan_performance.v1` records the next backend optimization immediately after module decoupling: collect Qt layer state, resolve renderer targets, compile visibility/opacity/blend/pick state, freeze static geometry batches, apply dirty flags, then submit one Taichi render/composite pass. The contract is visible in Qt (`Inspect: Render plan perf`), launch packets, renderer capabilities and handoff inspection, and it explicitly marks `runtime_optimization_applied=false` until the render loop is actually refactored.
