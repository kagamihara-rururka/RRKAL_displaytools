...
- `rrkal_displaytools.layer_operator_shortcuts.v1` documents Qt-first Photoshop-like layer operations: select, visibility, lock, opacity, blend, solo, restore, undo, reset and diagnostics actions. The contract is exposed through Qt launch/provenance packets, No-GUI launch packets, renderer capabilities and handoff inspection, and smoke gates verify solo/restore/undo actions.
