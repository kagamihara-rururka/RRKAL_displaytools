...
- Renderer layer runtime blend mode 目前已對 ADS-B aircraft 與 Pin marker 獨立 overlay 支援 Normal、Screen、Multiply、Overlay、Soft Light；vector/aggregate layer blend 仍列為後續項。
...
- Renderer layer runtime sync 下一步擴充 vector/aggregate layer blend mode 與更完整 renderer diagnostics；目前 visibility、支援圖層 opacity、aircraft/pin overlay blend 已可透過 `state/renderer_layer_runtime_state.json` 即時同步，Qt 與 renderer 端已有 bridge diagnostics、history、renderer ack 與 locked layer 防誤改。
