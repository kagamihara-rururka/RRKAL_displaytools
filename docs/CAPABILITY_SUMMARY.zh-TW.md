...
- Renderer 已支援 `--pin-file`、`--pin-json`、`--pin-layer`、`--pin-size`、`--pin-horizon-eps`。Qt 若有 Pins，啟動 renderer 時會以 `--pin-json` 傳入，renderer 會每 frame 投影並只繪製可見 hemisphere 上的 Pin marker；`selected_pin_id` 會以 profile-aware 外圈高亮。Pin marker 已接 scientific、nautical、tactical、parchment style profiles。
...
- Select 工具下一步是補 canvas/preview 操作回饋與圖層 hit target；Pin 下一步是補 globe mask / depth buffer refinement、label layout 與互動 pick；Brush/Mask 暫不納入本輪 UI。
