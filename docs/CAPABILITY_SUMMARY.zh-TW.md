...
- Layer capability matrix 下一步可把 runtime badge 做成更清楚的顏色/tooltip legend，並把 skip/error 統計放進 Properties 面板摘要。
