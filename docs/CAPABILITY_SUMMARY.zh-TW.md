...
- Renderer layer runtime opacity 目前已落地到 hydrology/boundary/scale/contours 與 ADS-B aircraft overlay alpha；vehicle icons 與其他 point/icon layer 仍列為後續項。
...
- Point/icon opacity 下一步處理 vehicle icons、Pin layer 與其他獨立 overlay，前提是 renderer 有可單獨合成的 frame overlay。
