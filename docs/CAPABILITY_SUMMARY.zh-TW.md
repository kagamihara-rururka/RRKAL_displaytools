...
- `rrkal_displaytools.render_compose_parity_smoke.v1` adds `scripts/render_compose_parity_smoke.ps1` as the contract-only parity entrypoint for the next compose-run merge optimization. It records required sequential/merged RGBA artifacts, zero-diff tolerance, optional manifest output and explicitly keeps pre-commit smoke source-only until merged candidate artifacts exist.
