...
- Renderer layer runtime sync 下一步擴充 blend mode 與更完整 renderer diagnostics；目前 visibility 與支援圖層 opacity 已可透過 `state/renderer_layer_runtime_state.json` 即時同步，Qt 與 renderer 端已有 bridge diagnostics、history、renderer ack 與 locked layer 防誤改。
- Point/icon opacity 下一步處理 vehicle icons 與其他獨立 overlay，前提是 renderer 有可單獨合成的 frame overlay。
