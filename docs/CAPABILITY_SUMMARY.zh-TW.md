...
- 左側 Tools dock 已新增科研導向 tool palette：Move、Select、Pin。Select 用於指定 active layer；Pin 用於科研標記意圖，可保存 type、label、note。Brush/Mask 暫不納入本輪 UI。`tool_state` 會寫入 profile 與 launch packet。
...
- Select 工具下一步是補完整圖層選取流程與 canvas/preview 操作回饋；Pin 下一步是接 canvas/globe hit-test 與經緯度欄位；Brush/Mask 暫不納入本輪 UI。
