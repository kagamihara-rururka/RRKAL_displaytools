...
- Renderer layer runtime sync 下一步擴充 opacity、blend mode 與更完整 renderer diagnostics；目前 visibility 已可透過 `state/renderer_layer_runtime_state.json` 即時同步，Qt 與 renderer 端已有 bridge diagnostics、history、renderer ack 與 locked layer visibility 防誤改。
...
- Boundary highlight 下一步是從目前 line hover outline/glow 升級到疆域/領海/EEZ feature identity、polygon fill mask，以及更完整 gamma/contrast shader。
