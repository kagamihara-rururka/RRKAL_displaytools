...
- Canvas Preview ???????? UI-only ???????? equirectangular canvas mapping?????? Pin ? latitude/longitude?????? selected pin ? marker ???renderer-facing cursor raycast helper ?? smoke ?? center hit / outside-globe miss?
