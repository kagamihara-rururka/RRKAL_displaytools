...
- `rrkal_displaytools.compose_run_parity_artifact_workflow.v1` exposes the parity artifact workflow through Qt render-plan inspection, launch packets, renderer capabilities and handoff inspection. It names `state/compose_parity`, baseline/candidate PNG paths, metadata output, precommit command and manual diff command, and records the remaining blocker: add an opt-in merged compose candidate renderer path before enabling runtime compose-run merging.
