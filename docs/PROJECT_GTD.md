...
| Display shell / canvas matrix | In progress | `display_core` now defines renderer-package-free `LayerModel`, `ViewModel`, `CanvasDescriptor`, `RendererEntry`, `register_renderer`, `lookup_renderers`, `build_canvas_registry_packet`, `build_sample_view_models_packet` and `build_display_shell_capability_packet` contracts. `scripts/inspect_display_shell_render_matrix.ps1` gives clone-first evidence for the DisplayShell + Canvas System + Layer System + Render Matrix direction without replacing the current globe runtime; `rrkal_displaytools.canvas_registry.v1` lists phase-1 `EarthCanvas` and `TimeSeriesCanvas` boundaries, `rrkal_displaytools.sample_view_models.v1` proves Earth / TimeSeries views share the same ViewModel formula, and contract-only `DisplayToolsGeoRendererAdapter` / `ContractOnlyTimeSeriesRendererAdapter` samples prove decorator registry discovery. | After render-plan composition seams stabilize, extract the current globe path as `EarthCanvas`, then add a minimal `TimeSeriesCanvas` adapter before broad renderer adapters. |
