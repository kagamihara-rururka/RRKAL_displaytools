...
| Qt operator UI | MVP | Added `rrkal_displaytools_qt_panel.py`, a PyQt6 control panel for layer toggles, style profiles, topo source, data mode, ocean material values, and renderer launch/stop. Tk is not the primary UI direction. | Wire deeper in-app layer control after the renderer loop is easier to separate. |
