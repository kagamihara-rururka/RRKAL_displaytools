...
| Ocean material and sea-state port | In progress | Ocean material controls and sea-condition diagnostics exist as contract-oriented scaffolding. The Qt panel exposes wave strength, roughness, foam, Ocean 3D safe preview, control-board audit visibility, and research-oriented performance budget presets. | Keep scalar controls stable until 7:00, then move render-plan merge / pass reduction into the decoupling phase. |
