...
| Cross-machine onboarding / CI smoke | MVP | Added Windows helper scripts: `scripts/setup_windows.ps1`, `scripts/run_qt_panel.ps1`, `scripts/render_quick_smoke.ps1`, `scripts/smoke.ps1`, and `scripts/list_visual_contract_inspectors.ps1` as a no-GUI reviewer command index. | Add richer troubleshooting only after another machine reports concrete setup friction. |
