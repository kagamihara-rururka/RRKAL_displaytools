...
| Display shell / canvas matrix | In progress | `display_core` now defines renderer-package-free `LayerModel`, `ViewModel`, `RendererEntry`, `register_renderer`, `lookup_renderers` and `build_display_shell_capability_packet` contracts. `scripts/inspect_display_shell_render_matrix.ps1` gives clone-first evidence for the DisplayShell + Canvas System + Layer System + Render Matrix direction without replacing the current globe runtime. | After render-plan composition seams stabilize, extract the current globe path as `EarthCanvas`, then add a minimal `TimeSeriesCanvas` contract before broad renderer adapters. |
