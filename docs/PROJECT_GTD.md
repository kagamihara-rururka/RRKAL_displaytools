...
| Module boundaries | In progress | Monolith contains diagnostics for extraction readiness and seam matrices. `decoupling_readiness.py` now records the pre-7:00 UI closure policy and post-7:00 extraction order; Qt and no-GUI reviewer packets expose it through `decoupling_readiness`. | Start with render-plan compose extraction after the 7:00 decoupling gate. |
